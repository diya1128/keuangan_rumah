...
function goalCollected(goal, d) {
  ...
  const dep = d.goalDeposits.filter((g) => g.goal_id === goal.id).reduce((s, g) => s + (Number(g.nominal) || 0), 0);
  return base + dep;
}
...
const TABS = [
  ...
  { key: "goals", label: "Tujuan", icon: Target },
...
function Fab({ onPick }) {
  ...
  return (
    <>
      <div className="pointer-events-none fixed inset-x-0 z-40 flex justify-center" style={{ bottom: 84 }}>
  ...
  return (
    <div className="pb-6">
      <div className="px-5 pb-16 pt-6" style={{ background: `linear-gradient(160deg, ${C.blue}, ${C.blueDark})`, borderBottomLeftRadius: 36, borderBottomRightRadius: 36 }}>
        ...
        <div className="mt-6">
          <div className="text-[13px]" style={{ color: "rgba(34,48,58,0.68)" }}>Saldo kamu</div>
          <div className="text-[32px] font-extrabold" style={{ color: C.ink, fontFamily: "'Baloo 2', sans-serif" }}>{formatMoney(balance)}</div>
        </div>
        <div className="mt-4 flex gap-3">
          <button onClick={() => openTx("EXPENSE")} className="flex flex-1 flex-col items-center gap-1.5 rounded-2xl py-3" style={{ background: "rgba(34,48,58,0.08)" }}>
            <ArrowDownLeft size={18} color={C.orange} />
            <span className="text-[11.5px] font-bold" style={{ color: C.ink }}>Pengeluaran</span>
          </button>
          <button onClick={() => openTx("INCOME")} className="flex flex-1 flex-col items-center gap-1.5 rounded-2xl py-3" style={{ background: "rgba(34,48,58,0.08)" }}>
            <ArrowUpRight size={18} color={C.green} />
            <span className="text-[11.5px] font-bold" style={{ color: C.ink }}>Pemasukan</span>
          </button>
          <button onClick={() => openTx("TRANSFER")} className="flex flex-1 flex-col items-center gap-1.5 rounded-2xl py-3" style={{ background: "rgba(34,48,58,0.08)" }}>
            <ArrowLeftRight size={18} color={C.mintDark} />
            <span className="text-[11.5px] font-bold" style={{ color: C.ink }}>Transfer</span>
          </button>
        </div>
      </div>
...
/* ============================================================
   GOALS
   ============================================================ */
function GoalForm({ editing, onSave, onClose }) {
  const [nama, setNama] = useState(editing?.nama || "");
  ...
  function submit() {
    ...
    onSave({ id: editing?.id || uid("goal"), nama: nama.trim(), target_nominal: t, current_nominal: editing?.current_nominal || 0, target_date: targetDate || null, icon, warna, status: editing?.status || "ACTIVE" });
  }

  return (
    <Sheet title={editing ? "Edit tujuan" : "Tujuan baru"} onClose={onClose} footer={<PrimaryButton onClick={submit}>Simpan tujuan</PrimaryButton>}>
      <Field label="Nama tujuan" required><TextInput value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Contoh: Dana Darurat" /></Field>
...
function GoalDepositForm({ goal, data, onSave, onClose }) {
  ...
  function submit() {
    ...
    const nom = Number(nominal);
    if (!nom || nom <= 0) return setErr("Nominal harus lebih dari 0.");
    onSave({ id: uid("gd"), goal_id: goal.id, account_id: accountId, nominal: nom, tanggal, catatan: catatan || null });
  }
...
function RealizeInstallmentForm({ item, data, onSave, onClose }) {
  const [accountId, setAccountId] = useState(data.accounts.find((a) => a.is_active)?.id || "");
...
function GoalsPage({ data, go, mutate }) {
  ...
  return (
    <div className="pb-6">
      <PageHeader title="Tujuan Keuangan" right={<button onClick={() => setForm({})} className="flex h-9 w-9 items-center justify-center rounded-full" style={{ background: C.blue, color: contrastText(C.blue) }}><Plus size={18} /></button>} />
      <div className="px-5">
        {data.goals.length === 0 ? (
          <EmptyState icon={Target} title="Belum ada tujuan" subtitle="Buat target menabung, misalnya Dana Darurat atau Liburan." />
        ) : (
          <div className="flex flex-col gap-3">
            {data.goals.map((g) => {
              const collected = goalCollected(g, data);
      ...
      {form && <GoalForm editing={form.id ? form : null} onSave={(payload) => { form.id ? mutate.updateItem("goals", payload.id, payload) : mutate.addItem("goals", payload); setForm(null); }} onClose={() => setForm(null)} />}
    </div>
  );
}
...
function GoalDetailPage({ data, id, back, mutate }) {
  ...
  const [confirmDel, setConfirmDel] = useState(false);
  if (!goal) return <div className="p-5"><PageHeader title="Tujuan" onBack={back} /><EmptyState title="Tujuan tidak ditemukan" /></div>;
  ...
  const deposits = data.goalDeposits.filter((d) => d.goal_id === goal.id).sort((a, b) => (a.tanggal < b.tanggal ? 1 : -1));
  ...
  function realize(item, payload) {
    const depositId = uid("gd");
    mutate.addItem("goalDeposits", { id: depositId, goal_id: goal.id, account_id: payload.account_id, nominal: payload.nominal, tanggal: payload.tanggal, catatan: `Cicilan ke-${item.urutan} dari ${item.total_kali}` });
  ...
  return (
    <div className="pb-6">
      <PageHeader title="Detail Tujuan" onBack={back} right={<button onClick={() => setForm(goal)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-sm"><Pencil size={16} color={C.ink} /></button>} />
      <div className="px-5">
        <div className="mb-4 rounded-3xl p-5" style={{ background: (goal.warna || C.blue) + "18" }}>
          ...
          <ProgressBar percent={pct} status={pct >= 100 ? "LEWAT" : "AMAN"} />
          <div className="mt-2 text-[12.5px] font-bold" style={{ color: goal.warna || C.blue }}>{pct}% tercapai</div>
        </div>
        {goal.status === "ACTIVE" && (
          <div className="mb-4 grid grid-cols-2 gap-2">
      ...
      {form && <GoalForm editing={form} onSave={(payload) => { mutate.updateItem("goals", payload.id, payload); setForm(null); }} onClose={() => setForm(null)} />}
      {depForm && <GoalDepositForm goal={goal} data={data} onSave={(payload) => { mutate.addItem("goalDeposits", payload); setDepForm(false); }} onClose={() => setDepForm(false)} />}
      ...
      {realizeItem && <RealizeInstallmentForm item={realizeItem} data={data} onSave={(payload) => realize(realizeItem, payload)} onClose={() => setRealizeItem(null)} />}
      {confirmDelPlanItem && (
        <ConfirmDialog
  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-5">
        ...
        <button onClick={() => setConfirmReset(true)} className="mt-6 w-full rounded-2xl py-3.5 text-[14px] font-bold" style={{ background: C.redLight, color: C.red }}>Hapus semua data</button>
      </div>
      <Toast message={saved ? "Perubahan disimpan" : ""} />
      {confirmReset && (
        <ConfirmDialog
          title="Hapus semua data?"
          message="Semua rekening, transaksi, budget, tagihan, dan tujuan akan dihapus permanen. Tindakan ini tidak bisa dibatalkan."
          danger
...
export default function App() {
  ...
  if (!data.settings.onboarded) {
    return (
      <Shell>
        <Onboarding onStart={(name) => mutate.setBulk({ settings: { ...data.settings, familyName: name, onboarded: true } })} />
      </Shell>
    );
  }
